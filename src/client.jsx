...
function ModelSelect({ models, preferences, savePreferences, type }) {
  const selected = preferences.selections[type] || "";
...
function ResultCard({ result, selected, setSelected, type, updateResult }) {
    ...
    <article class="result-card">
      <header>
        <label>
          <input
            checked={selected}
            disabled={!["completed", "failed"].includes(result.state)}
      ...
      <p>{result.error || result.prompt}</p>
      ...
      <footer class="inline-controls">
        ...
        <button class="contrast outline" disabled={!["completed", "failed"].includes(result.state)} onClick={() => action("", "DELETE")} type="button">Discard</button>
        ...
        <button class="secondary outline" disabled={result.state !== "failed"} onClick={() => action("/retry")} type="button">Retry</button>
      </footer>
    </article>
  );
}
...
function MediaWorkspace({
  ...
  const visibleModels = useMemo(
    () => models.filter((model) => `${model.name} ${model.id} ${model.description}`.toLowerCase().includes(search.toLowerCase())),
    [models, search],
  );
  ...
  function updateResult(updated) {
    setBatches((current) =>
      current.map((batch) => ({
        ...batch,
        results: batch.results.map((result) => (result.id === updated.id ? updated : result)),
      })),
    );
    reloadLibrary();
  }
  ...
  async function generate(event) {
    ...
    try {
      const batch = await api("/api/batches", {
        body: JSON.stringify({
          type,
          model: preferences.selections[type],
          prompt,
          quantity,
        }),
        headers: { "content-type": "application/json" },
  ...
  return (
    <div class="media-layout">
      <form class="composer" onSubmit={generate}>
        ...
        <ModelSelect models={visibleModels} preferences={preferences} savePreferences={async (next) => { await savePreferences(next); await reloadModels(type); }} type={type} />
        <TemplateTools prompt={prompt} reload={reloadTemplates} setPrompt={setPrompt} templates={templates} type={type} />
        <label>
          Prompt
          <textarea onInput={(event) => setPrompt(event.currentTarget.value)} rows={6} value={prompt} />
        </label>
        <label>
          Results per Batch
          <input max="50" min="1" onInput={(event) => setQuantity(Number(event.currentTarget.value))} type="number" value={quantity} />
        </label>
        <button disabled={!prompt.trim() || !preferences.selections[type]}>Generate Batch</button>
        <ErrorNotice error={localError || error} />
      </form>
      <section>
        <div class="review-bar">
          ...
          <div class="inline-controls">
            <button disabled={!selected.size} onClick={() => review("keep")} type="button">Keep selected</button>
        ...
        {batches.map((batch) => (
          <details open>
            <summary>{batch.prompt} · {batch.results.length} results</summary>
